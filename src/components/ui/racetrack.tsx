...
const GlowingTrack = memo(function GlowingTrack({ svgPath, color = '#00ff00', rotationSpeed = 0.001, maxFps = 30 }: GlowingTrackProps) {
  ...
  const points = useMemo(() => {
    ...
    const fullSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="144 144 512 512"><path d="${svgPath}"/></svg>`
    const data = loader.parse(fullSvg)
  ...
  useFrame((_: unknown, delta: number) => {
    ...
    if (glowMaterial && glowMaterial.uniforms) {
      glowMaterial.uniforms.time.value = elapsedRef.current
    }
  })
